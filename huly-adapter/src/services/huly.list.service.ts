...
export async function listIssues(projectId?: string, limit = 20) {
  const client: any = await getRestClient();

  try {
    const targetProjectId = projectId || getEnv("HULY_PROJECT_ID");
    ...
    const normalizedIssues = issues.map((issue: any) => {
      ...
      return {
        ...
        url: `${process.env.HULY_URL || "https://huly.app"}/workbench/${process.env.HULY_WORKSPACE_ID}/tracker/${project._id}/issues`
      };
    });

    normalizedIssues.sort((a: any, b: any) => {
      return Number(b.modifiedOn ?? 0) - Number(a.modifiedOn ?? 0);
    });
